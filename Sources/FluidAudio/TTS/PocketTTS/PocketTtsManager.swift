...
public actor PocketTtsManager {
    ...
    /// Run a closure inside the PocketTTS model store context.
    /// Used by the main app to access models for PersistentTTSSession warm-up.
    /// The closure runs synchronously within the actor-isolated model store.
    public func executeInModelStore<T: Sendable>(
