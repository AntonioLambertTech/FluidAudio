...
public actor PocketTtsManager {
    ...
    /// Build warm cache from carrier sentence during ringing.
    ...
    /// Streaming synthesis — pass conversationCache from previous chunk.
    /// Each chunk is cheaper and sounds more natural as the call progresses.
    public func synthesizeStreaming(
        text: String,
        voiceData: PocketTtsVoiceData,
        conversationCache: PocketTtsSynthesizer.KVCacheState?,
        warmCache: PocketTtsSynthesizer.KVCacheState
    ) async throws -> PocketTtsSynthesizer.StreamingSynthesisResult {
        guard isInitialized else {
            throw PocketTTSError.modelNotFound("PocketTTS not initialized")
        }
        return try await PocketTtsSynthesizer.withModelStore(modelStore) {
